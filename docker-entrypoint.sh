#!/bin/bash -xe
 ...
 if [[ ! -v SPARK_BINARY_URL ]]; then
   SPARK_BINARY_URL=http://apache.rediris.es/spark
 fi
 ...
 if [[ ! -v HADOOP_BINARY_URL ]]; then
   HADOOP_BINARY_URL=http://www.eu.apache.org/dist/hadoop/common
 fi
 ...
 if [[ "${EXECUTION_MODE}" == "mesos" || "${EXECUTION_MODE}" == "yarn" || "${EXECUTION_MODE}" == "standalone" ]]; then
   ...
   wget "${SPARK_BINARY_URL}/${SPARK_VERSION}/${SPARK_HADOOP_VERSION_FILE}"
   tar -xvzf ${SPARK_HADOOP_VERSION_FILE}
   rm ${SPARK_HADOOP_VERSION_FILE}
   wget "${HADOOP_BINARY_URL}/${HADOOP_VERSION}/${HADOOP_VERSION_FILE}"
